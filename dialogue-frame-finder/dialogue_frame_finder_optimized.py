"""
dialogue_frame_finder_optimized.py
==================================
CPU-optimized variant of dialogue_frame_finder.py.

Identical pipeline / architecture / matching logic as the original script --
only the inference configuration changed:

  * Default CPU model        : small     -> tiny.en   (English-only, fastest variant)
  * Long-tier coarse model   : small     -> tiny.en   (only needs approximate location)
  * Long-tier fine model     : small     -> tiny.en   (speed experiment mode)
  ...
  * transcribe(beam_size=5,
               condition_on_previous_text=False)
                                           full beam search (cheap on tiny), no
                                           hallucination loops
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Override the faster-whisper model for short/medium tier. "
             ...
             "Default is 'tiny.en' on CPU, 'large-v3' on GPU.",
    )
    parser.add_argument(
        "--coarse-model", default="tiny.en",
        help="Model used for the coarse pass on long-tier audio. (default: tiny.en)",
    )
    parser.add_argument(
        "--fine-model", default="tiny.en",
        help="Model used for the fine pass on the candidate window of long-tier audio. "
             "(default: tiny.en -- swap to large-v3 on GPU for higher accuracy)",
    )
    parser.add_argument(
        "--device", default=None,
...
def transcribe(audio_path: str, model_size: str = "tiny.en", speech_segments=None) -> dict:
    ...
    Optimized decoding (config only, no logic change):
      * beam_size=5                      full beam search restored: tiny.en is small
                                         enough to afford it, and beam search recovers
                                         the accuracy that greedy decoding costs
...
def main():
    ...
    # Default model -- tiny.en on CPU (English-only, fastest variant),
    # large-v3 on GPU
    default_model = "tiny.en" if DEVICE == "cpu" else "large-v3"
