"""
dialogue_frame_finder_base_en.py
================================
CPU-optimized variant of dialogue_frame_finder.py  (SUBMISSION BACKUP).

Measured on i5-1245U (CPU-only), 54-min video, query "My mind rebels at
stagnation": total 950.6s (~15.8 min) vs ~45-70 min est. with the original
'small' configuration. Frame matched at 326.15s (known-good reference: 324.68s).

Identical pipeline / architecture / matching logic as the original script --
only the inference configuration changed:

  * Default CPU model        : small     -> base.en   (English-only, ~2-3x faster)
  * Long-tier coarse model   : small     -> base.en   (only needs approximate location)
  * Long-tier fine model     : small     -> small.en  (accuracy where it matters)
  * GPU default (unchanged)  : large-v3  -> large-v3
  * WhisperModel(cpu_threads=...)          uses real cores instead of CT2 default 4
  * transcribe(beam_size=1,
               condition_on_previous_text=False)
                                           greedy decode, no hallucination loops
...
7. Extract the frame at the matched timestamp  ->  save as JPEG
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Directory where the matched frame JPEG and JSON result are saved. "
             "Created automatically if it does not exist. (default: ./output)",
    )
    parser.add_argument(
        "--model", default=None,
        help="Override the faster-whisper model for short/medium tier. "
             "E.g. tiny.en, base.en, small.en, medium, large-v3. "
             "Default is 'base.en' on CPU, 'large-v3' on GPU.",
    )
    parser.add_argument(
        "--coarse-model", default="base.en",
        help="Model used for the coarse pass on long-tier audio. (default: base.en)",
    )
    parser.add_argument(
        "--fine-model", default="small.en",
        help="Model used for the fine pass on the candidate window of long-tier audio. "
             "(default: small.en -- swap to large-v3 on GPU for higher accuracy)",
    )
    parser.add_argument(
        "--device", default=None,
    ...
    parser.add_argument(
        ...
        help="Number of CPU threads for faster-whisper (ctranslate2). "
             "Defaults to min(8, logical cores). Only used when device is 'cpu'.",
    )
    parser.add_argument(
...
TIER_MODEL_MAP: dict = {}
LONG_COARSE_MODEL: str = "base.en"
LONG_FINE_MODEL: str = "small.en"
...
def load_whisper_model(model_size: str) -> WhisperModel:
    if model_size not in _MODEL_CACHE:
        ...
        _MODEL_CACHE[model_size] = WhisperModel(
            model_size, device=DEVICE, compute_type=compute, **kwargs
        )
    else:
        ...
        if source.startswith("http://") or source.startswith("https://"):
            ...
            for attempt_index, (attempt_args, attempt_label) in enumerate(download_attempts):
                ...
                for cmd_base in (
                    ...
                    cmd = cmd_base + ["-f", fmt] + attempt_args + ["-o", dest, source]
                    try:
...
@timed
def transcribe(audio_path: str, model_size: str = "base.en", speech_segments=None) -> dict:
    """
    ...
    Optimized decoding (config only, no logic change):
      * beam_size=1                      greedy decode, ~1.5-2x faster than beam 5
      * condition_on_previous_text=False prevents repetition/hallucination loops
                                         common with movie-style audio clips
    """
    try:
        ...
        # NOTE: faster-whisper's clip_timestamps default is "0", not None -- passing
        # None explicitly crashes generate_segments ('NoneType' object is not
        # iterable). Only pass it when we actually have VAD segments; otherwise
        # let the library default apply (internal VAD runs via vad_filter=True).
...
def extract_frame(video_path: str, timestamp_sec: float, fps: float, out_path: str) -> dict:
    """Seek to timestamp, read the frame, write it as JPEG."""
    try:
...
def run_pipeline(video_url_or_path: str, target_dialogue: str) -> dict:
    ...
    if tier_info["tier"] != "long":
        ...
        # NOTE: coarse pass uses vad_filter=True (not clip_timestamps).
        # Constraining to pre-computed VAD windows changes segment boundaries
        # and can cause the coarse pass to miss the target dialogue entirely.
...
def main():
    ...
    global TIER_MODEL_MAP, LONG_COARSE_MODEL, LONG_FINE_MODEL, CPU_THREADS
    global COOKIES_FROM_BROWSER, COOKIES_FILE
    ...
    # CPU threads for ctranslate2 (hybrid P/E-core CPUs saturate well before
    # the logical-core count, hence the cap at 8)
    CPU_THREADS = args.cpu_threads or min(8, os.cpu_count() or 4)
    thread_note = f" (threads: {CPU_THREADS})" if DEVICE == "cpu" else ""
    print(f"[INFO] Using device: {DEVICE}{thread_note}")

    # Default model -- base.en on CPU (English-only, fastest good-quality option),
    # large-v3 on GPU
    default_model = "base.en" if DEVICE == "cpu" else "large-v3"
